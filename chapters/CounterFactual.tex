...
Counterfactual explanations provide a natural, user-focused approach to explaining individual model decisions. 
...
that flips the original prediction. Following prior work in entity matching \cite{Wang2022Minun}, 
we focus on counterfactual explanations generated through minimal textual perturbations of entity records, 
producing \emph{local} explanations for individual record pairs rather than global model interpretability.
...
while the other record remains fixed throughout the attack. This is achieved via a templated input with 
...
This strategy supports attribute-level interpretability by tracing perturbations to specific fields, 
...
Each perturbation represents a counterfactual explanation with a small, targeted change that flips 
...
through iterative, query-based perturbations of individual inputs \cite{morris2020textattack}. As a result, 
...
which remain unperturbed under the applied text-based attack space. However, this limitation is less critical in the context of the DBLP--ACM dataset, 
which is dominated by textual attributes such as authors, titles, and venues, making text-based counterfactual generation 
...
Despite these limitations, the approach is well suited for counterfactual argumentation in entity matching.
By producing localized, minimal perturbations, it enables direct inspection of attribute-level model
sensitivities and supports interpretable reasoning about individual matching decisions.